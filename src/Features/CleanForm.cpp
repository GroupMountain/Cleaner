...
bool shouldClean(gmlib::GMActor* actor,
    ...
    bool checkTag, bool checkItemAge, bool checkItemType, bool checkEffectArrow
) {
...
void ConfirmForm(Player& player, bool isOnline, std::string_view TypeName,
    bool checkTamed, bool checkTrusting, bool checkCustomName,
    bool checkTag, bool checkItemAge, bool checkItemType, bool checkEffectArrow
) {
    ...
    auto fm = ll::form::ModalForm(
        ...
        tr( "cleaner.form.confirm.warning", {
            tr(typeNameToLocString(typeNameStr)), typeNameStr,
            S(isOnline ? count_if(ll::service::getLevel()->getRuntimeActorList(),
            [&](const Actor* a) { return a && a->getTypeName() == typeNameStr && shouldClean((gmlib::GMActor*)a,checkTamed,checkTrusting,checkCustomName,
        checkTag,checkItemAge,checkItemType,checkEffectArrow); })
    ...
    fm.sendTo(player, [=](const Player& p, ll::form::ModalFormResult rst, ll::form::FormCancelReason res) {
        ...
        if (isOnline) {
            for (auto* en : ll::service::getLevel()->getRuntimeActorList()) {
                if (en->getTypeName() == typeNameStr && shouldClean((gmlib::GMActor*)en,
                    checkTamed, checkTrusting, checkCustomName, checkTag, checkItemAge, checkItemType, checkEffectArrow)) {
                    en->despawn();
                }
            }
        } else{
            std::thread([=]() {
                {
                    ...
                    if (it != OfflineEntity.offlineEntities.end()) {
                        for (auto& en : it->second) {
                            if (en.isValid()&&(
                                shouldCleanUnload(en,checkTamed,checkTrusting,checkCustomName,checkTag,checkItemAge,checkItemType,checkEffectArrow)))
                                en.remove();
...
void SettingCleanForm(Player& player, bool isOnline, std::string_view TypeName) {
    ...
    const auto toggles = [&] {
        ...
        if (typeNameStr == "minecraft:arrow") {
            v.insert(v.end(), {{"cleanEffectArrow", tr("cleaner.form.setting.cleanEffectArrow")}});
        }
        return v;
    }();
    ...
    fm.sendTo(player, [=](Player& p, const ll::form::CustomFormResult& rst, ll::form::FormCancelReason res) {
        ...
        bool cleanEffectArrow = typeNameStr == "minecraft:arrow"?std::get<uint64_t>(rst->at("cleanEffectArrow")):false;
        ConfirmForm(p, isOnline, typeNameStr,
            !cleanTamed, !cleanTrusting, !cleanCustomName,
            !cleanTag, !cleanItemAge, !cleanItemType,!cleanEffectArrow);
    });
}
...
void OnlineEntityForm(Player& player) {
    ...
    for (auto const& t : entityTypes) {
        statesPtr->push_back(ButtonState{
            t,
            std::make_shared<ObservableBoolean>(true)
        });
    }

    auto const subscription = keyword.subscribe(
        [statesPtr](std::string const& kw) {
            ...
            for (auto& bs : *statesPtr) {
                ...
                bs.visible->setData(
                    lowerTitle.find(lowerKw) != std::string::npos ||
                    lowerTitle1.find(lowerKw) != std::string::npos
                );
            }
        }
    );
    ...
    for (auto& bs : *statesPtr) {
        ...
        formPtr->button(
            tr("cleaner.form.common.button", {
                tr(typeNameToLocString(bs.type)),
                S(getEntityCountByType(bs.type))
            }),
            [formPtr, playerUuid=player.getUuid(), type]() {
                ...
                // 第二步：打开新表单（检查玩家是否在线）
                if (auto* pl =ll::service::getLevel()->getPlayer(playerUuid))
                    SettingCleanForm(*pl, true, type);
            },
            {.visible = *bs.visible}
        );
    }
...
void OfflineEntityForm(Player& player) {
...

    {
        ...
        for (auto const& t : counts) {
            statesPtr->push_back(ButtonStateUnload{
                t.first,
                std::make_shared<ObservableBoolean>(true),
                t.second,
            });
        ...
        for (auto& bs : *statesPtr) {
            form->button(tr("cleaner.form.common.button",{
                tr(typeNameToLocString(bs.type)), S(bs.count)
            }), [title=bs.type,uuid,form]() {
                form->close();
                if (auto* pl =ll::service::getLevel()->getPlayer(uuid))
                    SettingCleanForm(*pl, false, title);
            }, {.visible = *bs.visible});
        }
    }
    if (const auto started = form->show(); !started)
        keyword.unsubscribe(subscription);
}
